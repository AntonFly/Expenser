...
public class ExpenseTypeServiceImpl implements ExpenseTypeService {
    ...
    @Override
    public Collection<ExpenseTypeResponse> findAll() {
        return ((Collection<ExpenseType>) expenseTypeRepository.findAll()).stream()
                .map(ExpenseType::toResponse).toList();
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public ExpenseTypeResponse createType(ExpenseTypeCreate dto) {
        ExpenseType expenseType = new ExpenseType();
        expenseType.setName(dto.name());
        expenseType.setDescription(dto.description());
        return this.expenseTypeRepository.save(expenseType).toResponse();
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public ExpenseTypeResponse updateType(ExpenseTypeUpdate dto) throws ExpenseTypeNotFoundException {
